...
!******************************************************************
   ...
   ! CALL chdir("../example_data/HBout3G/")
   ! call getcwd(mydir) 
   ! pathsph = mydir
...
contains
    ...
    subroutine setup_input(rank)
        ...
        !-----------------------------------internal
        integer :: f, ios, cc, g
        character(LEN=50) :: stem, ranks, rankstr
        logical :: ISOPEN, OK
        character(len=400) :: ioemsg
        ...
        case ('datfile')
            ...
            case ('part', 'effC', 'hadr')
                ...
                infile1 = trim(pathsph)//'/contributions/higgsbounds-5.10.2/example_data/HBout3G/HB3-'
               ...
               write(*,*)'before ', trim(infile1)//trim(stem)//'_'//trim(rankstr)//'.dat'
               ! open (f, file=trim(infile1)//trim(stem)//'.dat', status='old', action='read', iostat=ios)
              
               open(f, file=trim(infile1)//trim(stem)//'_'//trim(rankstr)//'.dat', status='old', action='read', &
                      & iostat=ios, iomsg=ioemsg)
               inquire(unit=f, opened=ISOPEN)
               ...
               !print*,'after ', f, ios
               !write(*,*)'after ', trim(infile1)//trim(stem)//'_'//trim(rankstr)//'.dat' 
               write(*,*) ' ' 
               ...
               if (ios .ne. 0) then
                ...
                !open (f, file=trim(infile1)//trim(stem)//'.dat', status='old', action='read', iostat=ios) ! DK
                ios = 2 ! DK
...
! -
        ...
        if(allocated(effC))  deallocate (effC)
        if(allocated(effC_Hc))  deallocate (effC_Hc)
        allocate (effC_Hc(ndat))
        allocate (effC(ndat))  
...
!      req= part * req
    ...
    !************************************************************
    subroutine readthefile(rank, n_stem)
        !************************************************************
        ...
        character(len=20) :: ranks, rankstr  ! DK
        ...
        if (needed .or. opt) then
            ...
            !infile1 = trim(pathsph)//'HB3-' 
            infile1 =  trim(pathsph)//'/contributions/higgsbounds-5.10.2/example_data/HBout3G/HB3-'  


            f = f_orig + n_stem
            !open(f, file=trim(infile1)//trim(stem)//'.dat', status='old', action='read', iostat=ios)          
             open(f, file=trim(infile1)//trim(stem)//'_'//trim(rankstr)//'.dat',status='old',action='read',iostat=ios)
...
!        endif
        ...
        inf2 = trim(pathsph)//'/contributions/higgsbounds-5.10.2/example_data/HBout3G/HB3-'
               !trim(pathsph)//'HB3-'
        inf1 = trim(pathsph)//'/contributions/higgsbounds-5.10.2/example_data/HBout3G/HB3-'
               !trim(pathsph)//'HB3-'
